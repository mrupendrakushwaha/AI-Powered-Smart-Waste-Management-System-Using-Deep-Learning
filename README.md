# ♻️ AI-Powered Smart Waste Management System Using Deep Learning

## 📌 Project Overview

This project is an **AI-powered Smart Waste Management System** developed using **EfficientNetB0 Deep Learning**. It automatically classifies waste from **Camera 📷** or **Gallery 🖼️** images and provides **CO₂ Savings 🌱**, **Green Points 🏆**, **Dashboard 📊**, and **Professional PDF Report 📄**.

---

# ✨ Features

✅ 📷 Camera Input (Back Camera)

✅ 🖼️ Gallery Input (Multiple Images)

✅ 🤖 AI Waste Classification

✅ 📊 Interactive Dashboard

✅ 🥧 Pie Chart Analysis

✅ 📈 Bar Chart Analysis

✅ 🌱 CO₂ Savings Estimation

✅ 🏆 Green Points Calculation

✅ ⭐ Green Score

✅ ♻️ Sustainability Analysis

✅ 📄 PDF Report Generation

✅ 📁 CSV Report Generation

✅ 🔄 Reset Data

---

# 🗂️ Waste Categories

🟦 Plastic

📄 Paper

🍾 Glass

🥫 Metal

📦 Cardboard

🗑️ Trash

---

# 🛠️ Technologies Used

🐍 Python

🤖 TensorFlow

🧠 EfficientNetB0

📊 Pandas

🔢 NumPy

📈 Matplotlib

☁️ Google Colab

📄 ReportLab

---

# 🔄 Project Workflow

📷 Image Input

⬇️

🖼️ Image Preprocessing

⬇️

🤖 EfficientNetB0 Model

⬇️

♻️ Waste Classification

⬇️

🌱 CO₂ Savings Estimation

⬇️

🏆 Green Points

⬇️

📊 Dashboard

⬇️

📄 PDF Report

---
# 📸 Project Demonstration

## 📥 Input Images

| Cardboard | Glass | Metal |
|------------|--------|--------|
| ![](Input/Cardboard.png) | ![](Input/Glass.png) | ![](Input/Metal.png) |

| Paper | Plastic | Trash |
|--------|----------|-------|
| ![](Input/Paper.png) | ![](Input/Plastic.png) | ![](Input/Trash.png) |

---

## 📤 Prediction Outputs

| Cardboard | Glass | Metal |
|------------|--------|--------|
| ![](Output/Cardboard_output.jpg) | ![](Output/Glass_output.jpg) | ![](Output/Metal_output.jpg) |

| Paper | Plastic | Trash |
|--------|----------|-------|
| ![](Output/Paper_output.jpg) | ![](Output/Plastic_output.jpg) | ![](Output/Trash_output.jpg) |

# 🌍 Sustainability Impact

♻️ Promotes Recycling

🌱 Reduces CO₂ Emissions

🌎 Supports Clean Environment

🗑️ Smart Waste Segregation

🎯 Supports Sustainable Development Goals (SDGs)

---
# 📊 Dashboard

### Waste Distribution
![](Waste_Distribution.png)

### Waste Count
![](Waste_Count.png)

---

# 📄 Generated PDF Report

Download the sample report from the repository:

📄 **AI_Smart_Waste_Report.pdf**

# 📊 Project Outputs

✅ Waste Prediction

🎯 Confidence Score

📊 Dashboard Analytics

📄 PDF Report

📁 CSV Report

🏆 Green Score

---

# 🚀 Future Scope

📹 CCTV-based Waste Detection

📱 Android Application

☁️ Cloud Database Integration

📍 GPS-enabled Smart Monitoring

🗣️ Voice Assistant

🗑️ Smart IoT Dustbin

---

# 📂 Dataset

📦 **TrashNet Dataset**

### Waste Classes

♻️ Plastic

📄 Paper

🍾 Glass

🥫 Metal

📦 Cardboard

🗑️ Trash

---

# ▶️ How to Run

1️⃣ Open Google Colab

2️⃣ Upload Dataset

3️⃣ Run All Cells

4️⃣ Select Camera or Gallery

5️⃣ View Dashboard

6️⃣ Generate PDF Report

---

# 📁 Project Structure

```text
AI-Powered-Smart-Waste-Management-System-Using-Deep-Learning
│
├── 📓 AI_Powered_Smart_Waste_Management_System.ipynb
├── 📖 README.md
├── 📋 requirements.txt
├── ⚖️ LICENSE
├── 📂 dataset
├── 📂 reports
└── 📂 screenshots
```

---

# 👨‍💻 Author

**Upendra Kumar Kushwaha**

🎓 B.Tech (Artificial Intelligence & Machine Learning)

🏫 Shri Ram Institute of Technology, Jabalpur

---

# 📜 License

📚 This project is developed for **Educational and Sustainability Purposes**.

---

# ⭐ If you like this project

🌟 Star this repository

🍴 Fork this repository

💙 Support Sustainable AI
